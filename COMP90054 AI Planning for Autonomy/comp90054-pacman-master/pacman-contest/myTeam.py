...
def createTeam(firstIndex, secondIndex, isRed,
               first='AstarAgent1', second='Defender'):
    """
...
class DummyAgent(CaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        self.red_escape = self.choose_escape(gameState)[0]
        self.blue_escape = self.choose_escape(gameState)[1]
        self.enemy_direction = self.enemy_direction(gameState)
    ...
    def dis_to_home(self, gameState):
        myposition = gameState.getAgentState(self.index).getPosition()
        dis_to_home = []
        for home in self.homeboundary:
            dis_to_home.append(self.getMazeDistance(myposition,home))
        return min(dis_to_home)

    def capsuleorhome(self, gameState, status):
        myposition = gameState.getAgentState(self.index).getPosition()
        if len(self.capsulelist) > 0:
            distance = 99999
            distance1 = 99999
            for capsule in self.capsulelist:
                if self.getMazeDistance(myposition, capsule) < distance:
                    distance = self.getMazeDistance(myposition, capsule)
            for home in self.homeboundary:
                if self.getMazeDistance(myposition,home) < distance1:
                    distance1 = self.getMazeDistance(myposition, home)
    ...
    def choose_escape(self,gameState):
        x_red = 1
        y_red = 2
        x_blue =self.width - 2
        y_blue = 0
        ...
        blue_escape = []
        if self.red:
            while y_red < self.height:
                if (x_red, y_red) not in (self.walls or self.myfood):
                    red_escape.append((x_red, y_red))
                y_red += 1
            while y_blue < self.height - 3:
                if (x_blue, y_blue) not in (self.walls or self.food):
                    blue_escape.append((x_blue, y_blue))
                y_blue += 1
        else:
             while y_red < self.height:
                if (x_red, y_red) not in (self.walls or self.food):
                    red_escape.append((x_red, y_red))
                y_red += 1
             while y_blue < self.height - 3:
                if (x_blue, y_blue) not in (self.walls or self.myfood):
                    blue_escape.append((x_blue, y_blue))
                y_blue += 1
        return red_escape, blue_escape

    def enemy_direction(self, gameState):

        # False means Left direction
        direction = 0
        myposition = (x, y) = gameState.getAgentState(self.index).getPosition()
        # print(myposition)
        enemies = [gameState.getAgentState(enemy) for enemy in self.getOpponents(gameState)]
        ...
        for enemy in enemies:
            if enemy.getPosition() is not None:
                if self.getMazeDistance(myposition, enemy.getPosition()) < distance:
                    (a, b) = enemy.getPosition()
    ...
    def FoodHeuristic(self, state, gameState):
        myposition = state
        foodposition = self.getFood(gameState).asList()
        # print("foodposition",foodposition)
        if len(foodposition) != 0:
            food1 = foodposition[0]
            food2 = foodposition[0]
            max_dis = 0
            for current_food in foodposition:
                for select_food in foodposition:
                    distance = self.getMazeDistance(current_food, select_food)
                    if max_dis < distance:
                        max_dis = distance
                        food1 = current_food
                        food2 = select_food
            d1 = self.getMazeDistance(myposition, food1)
            d2 = self.getMazeDistance(myposition, food2)
            return max_dis + min(d1, d2)
        else:
            return 0

    def SearchGhostHeuristic(self,state, gameState):
        myposition = state
        heuristic = [0]
        ghostState = [gameState.getAgentState(enemy) for enemy in self.getOpponents(gameState)]
        ghostPosition = [enemy.getPosition() for enemy in ghostState if enemy.getPosition() != None]
        for ghost in ghostPosition:
            heuristic.append(self.getMazeDistance(myposition, ghost))
    ...
    def aStar_search(self, problem, gameState, heuristic=nullHeuristic):
        ...
        PQ.push((self.getCurrentObservation().getAgentState(self.index).getPosition(), [], 0), 0)  # state, total actions, total cost
        ...
        while not PQ.isEmpty():
            ...
            for next_state, action, cost in successors:
                total_cost = problem.getCostOfActions(actions + [action]) + heuristic(next_state, gameState)
                redo_flag = True

                for (state_tmp, cost_tmp) in closed:
                    if (next_state == state_tmp) and (total_cost >= cost_tmp): redo_flag = False

                """Redo process."""
                if redo_flag:
                    PQ.push((next_state, actions + [action], total_cost),
                            total_cost + heuristic(next_state, gameState))
                    closed.append((next_state, total_cost))
        return None
...
class AstarAgent1(DummyAgent):
    ...
    #             return gameState.getAgentState(enemy).scaredTimer
    #     return None
    def chooseAction(self, gameState):
        next_safe_food = []
        next_danger_food = []
        ...
        if (len(all_food_list) <= 2 and gameState.getAgentState(self.index).isPacman) or (gameState.getAgentState(self.index).isPacman and score > 10 and
            len(self.getFoodYouAreDefending(gameState).asList()) < 5) or \
                (gameState.data.timeleft < 150 and gameState.getAgentState(self.index).isPacman and
                 gameState.getAgentState(self.index).numCarrying >= 3):
            if self.timer != 0:
                if myposition == (1,1) or myposition == (self.width - 2, self.height - 2):
                    self.timer = 0
                    self.history = []
                else:
                    self.timer -= 1
            ...
            if len(self.history) < 10:
                ...
                if self.isfreezed(self.history[-4],myposition,hasEnemies):
                    if self.timer == 0:
                        self.timer = 6
        ...
        # denfender
        if (len(all_food_list) == 0 and gameState.getAgentState(self.index).numCarrying == 0) or \
            (gameState.getAgentState(self.index).isPacman is False and hasEnemies and gameState.data.timeleft < 300 and score > 3) or \
                ((gameState.data.timeleft < 150 or len(self.getFoodYouAreDefending(gameState).asList()) < 5) and
                 gameState.getAgentState(self.index).isPacman is False and score > 1):
            be_eaten = False
        ...
        if len(all_food_list) == 0 and gameState.getAgentState(self.index).numCarrying > 0 and gameState.getAgentState(self.index).isPacman:
            # print("Successfully1")
            if self.timer != 0:
                if myposition == (1,1) or myposition == (self.width - 2, self.height - 2):
                    self.timer = 0
        ...
        if len(self.getCapsules(gameState)) != 0 and self.get_scaredtime(gameState) == 0 \
                and gameState.getAgentState(self.index).numCarrying >= 3 and hasEnemies is False:
            # print("Successfully3")
            if self.timer != 0:
                if myposition == (1,1) or myposition == (self.width - 2, self.height - 2):
                    self.timer = 0
            ...
            if len(self.history) < 10:
                ...
                if self.isfreezed(self.history[-4],myposition,hasEnemies):
                    ...
                    if self.timer != 0:
                        ...
                        self.history.append(self.get_next_position(gameState,actions[0]))
            # print("Capsule1:",actions[0])
            if actions is None or actions == []:
                return 'Stop'
            else:
                return actions[0]

        # if agent takes capsule. scared time >= 15
        # print("scaredTimer",self.get_scaredtime(gameState))
        if self.get_scaredtime(gameState) >= 15:
            # print("Successfully4")
            ...
            if actions is None or actions == []:
                ...
                # if carrying food is bigger than 15, go back home right now

        # if carrying food is bigger than 15, go back home right now
        if gameState.getAgentState(self.index).isPacman and gameState.getAgentState(self.index).numCarrying >= 15 or \
                (gameState.data.timeleft < self.dis_to_home(gameState) + 60):
            if self.timer != 0:
                if myposition == (1,1) or myposition == (self.width - 2, self.height - 2):
                    self.timer = 0
                    self.history = []
            ...
            if len(self.history) < 10:
                ...
                if self.isfreezed(self.history[-4],myposition,hasEnemies):
                    ...
                    if self.timer != 0:
                        ...
                        self.history.append(self.get_next_position(gameState,actions[0]))
            # print("back home3", actions[0])
            if actions is None or actions == []:
                return 'Stop'
            else:
                return actions[0]

        # if agent takes at least 8 points, search capsule or go home
        if gameState.getAgentState(self.index).isPacman and gameState.getAgentState(self.index).numCarrying > 7 and \
                (hasEnemies is False or self.get_scaredtime(gameState) < 15):
            if self.timer != 0:
                if myposition == (1,1) or myposition == (self.width - 2, self.height - 2):
                    self.timer = 0
                    self.history = []
        ...
        # if our agent takes at least 1 points and meet enemy, then search capsule or go home
        if gameState.getAgentState(self.index).isPacman and gameState.getAgentState(self.index).numCarrying >= 0 and hasEnemies:
            if self.timer != 0:
                if myposition == (1,1) or myposition == (self.width - 2, self.height - 2):
                    self.timer = 0
                    self.history = []
        ...
        # if our agent takes at most 7 points and doesn't meet enemy, eat food greedily
        if gameState.getAgentState(self.index).numCarrying <= 7 and hasEnemies is False and gameState.data.timeleft > 500:
            problem = FoodSearchProblem(gameState, self, self.index)
            actions = self.aStar_search(problem, gameState, self.GhostHeuristic)
...
class AstarAgent2(DummyAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for food in self.safe_food:
            if self.getMazeDistance(food, teamposition) <= 20:
                self.safe_food.remove(food)
                self.dangerous_food.append(food)
        ...
        # denfender
        if  len(all_food_list) == 0 and gameState.getAgentState(self.index).numCarrying == 0 or \
            (gameState.getAgentState(self.index).isPacman is False and hasEnemies and gameState.data.timeleft < 800) or \
                ((gameState.data.timeleft < 150 or len(self.getFoodYouAreDefending(gameState).asList()) < 5) and gameState.getAgentState(self.index).isPacman is False and score > 1):
            be_eaten = False
            my_pos = gameState.getAgentPosition(self.index)
            enemies = [gameState.getAgentState(i) for i in self.getOpponents(gameState)]
            enemy_Pos = []
        ...
        # print("scaredTimer",self.get_scaredtime(gameState))
        if self.get_scaredtime(gameState) != 0:
            # print("Successfully4")
            if self.get_scaredtime(gameState) >= 20: #dangerous > 0
                problem = FoodSearchProblem(gameState, self, self.index)
                actions = self.aStar_search(problem, gameState, self.FoodHeuristic)
            else:
                if self.get_scaredtime(gameState) < 10:
                    ...
                    problem = SearchGhostProblem(gameState, self, self.index)
                    actions = self.aStar_search(problem, gameState, self.SearchGhostHeuristic)
                # if hasEnemies:
                #     if len(self.getCapsules(gameState)) != 0:
                #         problem = SearchCapsule(gameState, self, self.index)
                #         actions = self.aStar_search(problem, gameState, self.nullHeuristic)#yaogai
                #     else:
                #         problem = BackHomeProblem(gameState, self, self.index)
                #         actions = self.aStar_search(problem, gameState, self.GhostHeuristic)
                # else:
        ...
        if gameState.getAgentState(self.index).numCarrying > 15 or gameState.data.timeleft < self.dis_to_home(gameState) + 60:
            problem = BackHomeProblem(gameState, self, self.index)
            actions = self.aStar_search(problem, gameState, self.GhostHeuristic)
            if actions is None or actions == []:
                return 'Stop'
    ...
    def chooseAction(self, gameState):
        ...
        if not flag1 and not flag2:
            ...
            for action in actions:
                ...
                # else:
                #     if my_pos[0] <= self.mid_x and my_pos != self.defense_point:
                #         print("Stop111", my_pos)
                #         print(self.defense_point)
                #         problem = BackDefenseProblem(gameState, self, self.index)
                #         actions = self.aStar_search(problem, gameState, self.nullHeuristic)
                #         select_actions.append(actions[0])
                #         new_state = gameState.generateSuccessor(self.index, actions[0])
                #         new_pos = new_state.getAgentPosition(self.index)
                #         values.append(self.getMazeDistance(new_pos, self.targetPosition))
                #     elif my_pos == self.defense_point:
            ...
            problem = FoodSearchProblem2(gameState, self, self.index)
            ...
            if (actions == [] or actions is None) and not gameState.getAgentState(self.index).isPacman:
                # print("Stop222")
                return "Stop"
            elif (actions == [] or actions is None) and gameState.getAgentState(self.index).isPacman:
                problem = BackHomeProblem(gameState, self, self.index)
                actions = self.aStar_search(problem, gameState, self.GhostHeuristic)
            return actions[0]
        else:
            ...
            actions = self.aStar_search(problem, gameState, self.GhostHeuristic)
            if (actions == [] or actions is None) and not gameState.getAgentState(self.index).isPacman:
                if my_pos != self.mid_x:
                    problem = BackDefenseProblem(gameState, self, self.index)
                    actions = self.aStar_search(problem, gameState, self.SuperHeuristic)
            if (actions == [] or actions is None) and gameState.getAgentState(self.index).isPacman:
                problem = BackHomeProblem(gameState, self, self.index)
                actions = self.aStar_search(problem, gameState, self.nullHeuristic)
            return actions[0]
...
class FoodSearchProblem2(PositionSearchProblem):
    def __init__(self, gameState, agent, agentIndex=0):
        for mem in agent.teammatesIndices:
            if mem is not agentIndex:
                teamposition = gameState.getAgentState(mem).getPosition()
        temp_food = agent.getFood(gameState).asList().copy()
        for food in temp_food:
            if agent.getMazeDistance(food, teamposition) <= 10:
                temp_food.remove(food)
        self.food = temp_food
        self.capsule = agent.getCapsules(gameState)
        self.startState = gameState.getAgentState(agentIndex).getPosition()
        self.walls = gameState.getWalls()
...
class EscapeStuckProblem_1(PositionSearchProblem):
    def __init__(self, gameState, agent, agentIndex):
        ...
        self.costFn = lambda x: 1
        self._visited, self._visitedlist, self._expanded = {}, [], 0  # DO NOT CHANGE
        self.escape_1 = agent.red_escape
        self.escape_2 = agent.blue_escape
        self.enemy_direction = agent.enemy_direction

    def isGoalState(self, state):
        # print(state)
        # print("State 111111", (state in self.escape_1) or (state in self.escape_2))
        # return (state in self.escape_1) or (state in self.escape_2)
        if self.enemy_direction == 1:
            return state in self.escape_1
        elif self.enemy_direction == 0:
            return state in self.escape_2
        else:
            return (state in self.escape_1) or (state in self.escape_2)
...
class SearchGhostProblem(PositionSearchProblem):
    def __init__(self, gameState, agent, agentIndex):
        ...
        self.costFn = lambda x: 1
        self._visited, self._visitedlist, self._expanded = {}, [], 0  # DO NOT CHANGE
        if agent.dis_to_ghost(gameState) != None:
            self.ghost = agent.dis_to_ghost(gameState)[1].getPosition()
        else:
            self.ghost = None
        # print(self.ghost)
    def isGoalState(self, state):
        if self.ghost != None:
            ...
            return state in self.food
class BackDefenseProblem(PositionSearchProblem):
    def __init__(self, gameState, agent, agentIndex=0):
        ...
        self._visited, self._visitedlist, self._expanded = {}, [], 0  # DO NOT CHANGE
        self.homeBoundary = agent.get_home_pos(gameState)
        self.defense_point = agent.mid_x
    ...
    def isGoalState(self, state):
        # the goal state is the boudary of home or the positon of capsule
        return (int(state[0]), int(state[1])) is self.defense_point
