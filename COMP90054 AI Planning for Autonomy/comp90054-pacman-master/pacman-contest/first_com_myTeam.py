        ...
        while not PQ.isEmpty():
            ...
            if state not in closed.keys() or cost < best_g[state]:
                ...
                for child, action, child_cost in successors:
                    ...
                    f_value = g + heuristic(child, gameState)
    ...
    def chooseAction(self, gameState):
        ...
        for enemy in self.enemiesIndices:
            ...
            if enemy_pos:
                ...
                if dis_to_me < dis_to_teammate and dis_to_me < 4:
                    hasEnemies = True

        # when time is limtted, go back to defence
        if (gameState.data.timeleft < 250) and gameState.getAgentState(self.index).isPacman \
                and (score > 1 or gameState.getAgentState(self.index).numCarrying >= 1):
            problem = BackHomeProblem(gameState, self, self.index)
            actions = self.aStar_search(problem, gameState, self.GhostHeuristic)
            return actions[0]

        # denfender
        if (gameState.data.timeleft < 250) and gameState.getAgentState(self.index).isPacman is False and score > 1:
            be_eaten = False
        ...
        if len(all_food_list) == 0 and gameState.getAgentState(self.index).numCarrying > 0:
            ...
            if len(self.history) < 10:
                ...
                if self.timer == 0 and self.isfreeze(self.history[-1:-11:-1]):
                    self.history.append(actions[0])
                elif self.isfreeze(self.history[-1:-11:-1]):
                    if self.timer != 0 :
                        actions = self.uniformcost_search(problem,gameState)
                        self.history.append(actions[0])
                    else:
                        actions = self.uniformcost_search(problem,gameState)
                        self.history.append(actions[0])
