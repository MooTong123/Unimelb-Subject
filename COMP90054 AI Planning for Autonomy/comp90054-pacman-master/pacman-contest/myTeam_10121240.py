...
class DummyAgent(CaptureAgent):
    ...
    def capsuleorhome(self, gameState):
        myposition = gameState.getAgentState(self.index).getPosition()
        status = False
...
class AstarAgent(DummyAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if (gameState.getAgentState(self.index).isPacman and score > 10 and \
                ...
                (gameState.data.timeleft < 150 and gameState.getAgentState(self.index).isPacman \
                and score > 5 and gameState.getAgentState(self.index).numCarrying >= 1) or \
            (gameState.data.timeleft < 100 and gameState.getAgentState(self.index).numCarrying >= 1):
        ...
        if  len(all_food_list) == 0 and gameState.getAgentState(self.index).numCarrying == 0 or \
            (gameState.getAgentState(self.index).isPacman is False and hasEnemies and gameState.data.timeleft < 300) or \
        ...
        if self.get_scaredtime(gameState) != 0:
            # print("scaredtime",self.get_scaredtime(gameState))
            if self.get_scaredtime(gameState) >= 15 and len(self.food) > 0:
        ...
        if gameState.getAgentState(self.index).numCarrying > 7 and (hasEnemies is False or self.get_scaredtime(gameState) < 15):
            if self.timer != 0:
                if myposition == (1,1) or myposition == (self.width - 2, self.height - 2):
                    self.timer = 0
                    self.history = []
                    ...
                    self.timer -= 1
            # status = self.capsuleorhome(gameState)
        ...
        if gameState.getAgentState(self.index).numCarrying <= 7 and hasEnemies is False:
            if self.timer != 0:
                if myposition == (1, 1) or myposition == (self.width - 2, self.height - 2):
                    self.timer = 0
                    self.history = []
