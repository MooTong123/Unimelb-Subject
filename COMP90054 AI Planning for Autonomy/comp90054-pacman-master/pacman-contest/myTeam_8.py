...
class AstarAgent(DummyAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if (gameState.data.timeleft < 150) and gameState.getAgentState(self.index).isPacman \
        ...
        if gameState.data.timeleft < 100 and gameState.getAgentState(self.index).isPacman and score > 1:
            problem = BackHomeProblem(gameState, self, self.index)
            actions = self.aStar_search(problem, gameState, self.GhostHeuristic)
            return actions[0]

        # denfender
        if ((gameState.data.timeleft < 150) and gameState.getAgentState(self.index).isPacman is False and score > 1):
            be_eaten = False
