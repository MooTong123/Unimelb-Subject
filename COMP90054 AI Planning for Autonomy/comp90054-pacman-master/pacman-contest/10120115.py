...
class AstarAgent(DummyAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if  len(all_food_list) == 0 and gameState.getAgentState(self.index).numCarrying == 0 or \
            (gameState.getAgentState(self.index).isPacman is False and hasEnemies and gameState.data.timeleft < 500) or \
        ...
        if len(all_food_list) == 0 and gameState.getAgentState(self.index).numCarrying > 0:
            myposition = gameState.getAgentState(self.index).getPosition()
            if self.timer != 0:
                if myposition == (1,1) or myposition == (self.width - 1, self.height - 1):
                    self.timer = 0
                    self.history = []
                else:
                    self.timer -= 1
        ...
        # if our agent takes at most 5 points and doesn't meet enemy, eat food greedily
