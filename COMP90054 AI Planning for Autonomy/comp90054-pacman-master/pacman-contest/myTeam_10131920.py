...
class DummyAgent(CaptureAgent):
    ...
    def ifCapsule(self,gameState,actions):
        # if the agent has a safe way to eat capsule
        next_positions = []
        (x,y) = gameState.getAgentState(self.index).getPosition()
        enemies = [gameState.getAgentState(enemy) for enemy in self.getOpponents(gameState)]
        ifCapsule = True
        ...
        print(actions)
        if actions is not None and actions is not []:
            ...
            for enemy in enemies:
                if enemy.getPosition() is not None:
                    for next_position in next_positions:
                        if self.getMazeDistance(next_position, enemy.getPosition()) <= 1:
                            ifCapsule = False
                        else:
                            ifCapsule = True
    ...
    def GhostHeuristic(self, state, gameState):
        ...
        ghost = []
        heuristic = 0
        penalty = [100, 1000, 4000, 6000, 99999]
        for enemy in enemies:
        ...
        if len(ghost) > 0:
            ...
            if distance <= 2:
                heuristic = distance + penalty[4 - distance]
        return heuristic
...
class AstarAgent1(DummyAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if len(self.getCapsules(gameState)) != 0 and self.get_scaredtime(gameState) == 0 \
            ...
            if self.ifCapsule(gameState,actions) is False:
                problem = BackHomeProblem(gameState, self, self.index)
                actions = self.aStar_search(problem, gameState, self.GhostHeuristic)
                ...
                if len(self.history) < 10:
                    ...
                    if self.isfreezed(self.history[-4], myposition, hasEnemies):
                        ...
                        if self.timer != 0:
                            ...
                            self.history.append(self.get_next_position(gameState, actions[0]))
                print("Capsule3 or back home:", actions[0])
            else:
                problem = BackHomeProblem(gameState, self, self.index)
                actions = self.aStar_search(problem, gameState, self.GhostHeuristic)
                if len(self.history) < 10:
