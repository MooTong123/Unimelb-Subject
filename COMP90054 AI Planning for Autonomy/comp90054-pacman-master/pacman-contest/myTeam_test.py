...
class Defender(DummyAgent):
    def registerInitialState(self, gameState):
        ...
        for food in self.food:
            if food[1] <= int(self.height / 2):
                self.low_foodlist.append(food)
            else:
                self.high_foodlist.append(food)
...
class EscapeStuckProblem_1(PositionSearchProblem):
    def __init__(self, gameState, agent, agentIndex):
        ...
        self._visited, self._visitedlist, self._expanded = {}, [], 0  # DO NOT CHANGE
        self.escape_1 = (1,1)
        self.escape_2 = (agent.width - 1, agent.height - 1)
        # print("escape_2",self.escape_2)

    def isGoalState(self, state):
        return (state == self.escape_1) or (state == self.escape_2)
